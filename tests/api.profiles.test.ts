...
describe("security profiles", () => {
  it("read_only refuses write runs with 403", async () => {
    ...
    const res = await app.inject({ method: "POST", url: "/api/runs", headers: auth(), payload: { prompt: "touch it", mode: "write" } });
  ...
  it("review_before_write creates an approval instead of a run, and approving launches it", async () => {
    ...
    const res = await app.inject({ method: "POST", url: "/api/runs", headers: auth(), payload: { prompt: "change something", mode: "write" } });
    expect(res.statusCode).toBe(202);
    const body = res.json() as { runId: string | null; status: string; pendingApproval: { id: string; kind: string } };
    expect(body.runId).toBeNull();
    expect(body.status).toBe("waiting_approval");
    expect(body.pendingApproval.kind).toBe("write_run");
    expect(ctx.runs.list({ limit: 50 }).some((r) => r.promptSummary.includes("change something"))).toBe(false);

    const pending = await app.inject({ method: "GET", url: "/api/approvals", headers: auth() });
    expect((pending.json() as Array<{ id: string }>).some((a) => a.id === body.pendingApproval.id)).toBe(true);

    const resolved = await app.inject({ method: "POST", url: `/api/approvals/${body.pendingApproval.id}/resolve`, headers: auth(), payload: { decision: "approved" } });
  ...
  it("denying a write_run approval launches nothing", async () => {
    setProfile("review_before_write");
    const res = await app.inject({ method: "POST", url: "/api/runs", headers: auth(), payload: { prompt: "denied one", mode: "write" } });
    const id = (res.json() as { pendingApproval: { id: string } }).pendingApproval.id;
    const resolved = await app.inject({ method: "POST", url: `/api/approvals/${id}/resolve`, headers: auth(), payload: { decision: "denied" } });
  ...
  it("controlled_write launches write runs immediately", async () => {
    ...
    const res = await app.inject({ method: "POST", url: "/api/runs", headers: auth(), payload: { prompt: "apply now", mode: "write" } });
    expect(res.statusCode).toBe(200);
    expect((res.json() as { runId: string }).runId).toMatch(/^[0-9a-f-]{36}$/);
  });

  it("routines write only under approved_automation", async () => {
    ctx.routines.save({
