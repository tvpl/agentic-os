...
/**
 * Whether the OS-level unit generated by `planStartupService` is present on
 * disk. Presence of the file is the signal the scheduler uses to label
 * routines as running under the `service` runner (vs. a bare `local` process).
 */
export function isStartupServiceInstalled(
  paths: MordomoPaths,
  platform: StartupPlatform = process.platform,
  homeDir: string = os.homedir(),
): boolean {
  try {
    const plan = planStartupService(paths, process.execPath, platform, homeDir);
    return plan.files.some((f) => fs.existsSync(f.path));
  } catch {
    return false;
  }
}
