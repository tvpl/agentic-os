...
function readPidInfo(ctx: AppContext): { pid: number; port: number; startedAt: number } | null {
  const pidFile = path.join(ctx.paths.run, "server.pid");
  try {
    const info = JSON.parse(fs.readFileSync(pidFile, "utf8")) as { pid: number; port: number; startedAt: number };
...
${pc.bold("MordomoOS")} — local agentic OS over Claude Code, Cursor Agent and Codex
  ...
  mordomo doctor           Full diagnostic (providers, auth, index, routines, security)
  mordomo start            Start the local service + Command Centre (add --foreground to stay attached)
  mordomo stop             Stop the local service
  mordomo status           Service status
  mordomo index            Re-index the workspace and regenerate memory routers
  mordomo sync [dir]       Compile canonical skills/routers to provider-native files (--apply to write)
  mordomo run <skill>      Run a skill headlessly (--provider, --model, --effort, --input k=v)
  ...
  mordomo service          Startup service: mordomo service install | remove | plan
...
async function cmdSetup(): Promise<void> {
  ...
  const nonInteractive = flags.has("--defaults") || !process.stdout.isTTY;
  ...
  const detections = {} as Record<ProviderId, { installed: boolean; version: string | null; auth: string; notes: string[] }>;
  ...
  if (nonInteractive) {
    ...
    if (!settings.providers[settings.defaultProvider].enabled) {
      const firstEnabled = (["claude", "cursor", "codex"] as ProviderId[]).find((i) => settings.providers[i].enabled);
    ...
    const enabled = await p.multiselect({
      ...
      options: (["claude", "cursor", "codex"] as ProviderId[]).map((id) => ({
        value: id,
        label: `${id}${detections[id].installed ? "" : " (not installed — can be enabled later)"}`,
      })),
      initialValues: (["claude", "cursor", "codex"] as ProviderId[]).filter(
    ...
    for (const id of ["claude", "cursor", "codex"] as ProviderId[]) {
      settings.providers[id].enabled = (enabled as ProviderId[]).includes(id);
    }

    // 3. Default provider
    const def = await p.select({
      message: "Default provider?",
      options: (enabled as ProviderId[]).map((id) => ({ value: id, label: id })),
      initialValue: (enabled as ProviderId[]).includes(settings.defaultProvider)
        ? settings.defaultProvider
        : (enabled as ProviderId[])[0],
    });
    if (p.isCancel(def)) return cancel();
    settings.defaultProvider = def as ProviderId;

    // 4. Model + effort per provider
    for (const id of enabled as ProviderId[]) {
      const models = await ctx.adapters[id].listModels();
      const model = await p.select({
        message: `Default model for ${id}?`,
        options: [
          { value: "", label: "(provider default)" },
          ...models.map((m) => ({ value: m.id, label: `${m.label}${m.recommendedFor ? pc.dim(` — ${m.recommendedFor}`) : ""}` })),
        ],
      ...
      settings.providers[id].defaultEffort = effort as Settings["providers"]["claude"]["defaultEffort"];
    }
    ...
    if ((foldersRaw as string).trim()) {
      ...
      for (const raw of (foldersRaw as string).split(",").map((s) => s.trim()).filter(Boolean)) {
    ...
    const extras = (extraExcludes as string).split(",").map((s) => s.trim()).filter(Boolean);
    settings.excludes = [...new Set([...settings.excludes, ...extras])];

    // 7. Identity
    const name = await p.text({ message: "System name?", defaultValue: settings.systemName, placeholder: settings.systemName });
    ...
    const port = await p.text({ message: "Local port?", defaultValue: String(settings.port), placeholder: String(settings.port) });
    ...
    const tz = await p.text({
      ...
      defaultValue: settings.timezone === "UTC" ? Intl.DateTimeFormat().resolvedOptions().timeZone : settings.timezone,
    ...
    const doSmoke = await p.confirm({ message: "Run a safe read-only smoke test for each enabled provider now?", initialValue: true });
    ...
    const auto = await p.confirm({
      message: "Configure automatic start at login? (creates an OS startup service — nothing is installed without this approval)",
  ...
  for (const check of report.checks) {
    const icon =
      check.status === "ok" ? pc.green("✔") : check.status === "warn" ? pc.yellow("!") : check.status === "fail" ? pc.red("✘") : pc.dim("-");
    p.log.message(` ${icon} ${check.label.padEnd(20)} ${pc.dim(check.detail)}`);
  }
  p.outro(
...
async function cmdDoctor(): Promise<void> {
  ...
  for (const check of report.checks) {
    const icon =
      check.status === "ok" ? pc.green("✔") : check.status === "warn" ? pc.yellow("!") : check.status === "fail" ? pc.red("✘") : pc.dim("−");
    console.log(` ${icon} ${check.label.padEnd(22)} ${check.detail}`);
  }
...
async function cmdStart(): Promise<void> {
  ...
  const out = fs.openSync(path.join(ctx.paths.logs, "service.out.log"), "a");
  ...
  else console.log(`${pc.red("✘")} Service did not report healthy — check ${path.join(ctx.paths.logs, "service.out.log")}`);
  ctx.close();
}
...
async function cmdStatus(): Promise<void> {
  ...
  if (info) {
    console.log(`${pc.green("●")} running · pid ${info.pid} · http://127.0.0.1:${info.port} · since ${new Date(info.startedAt).toLocaleString()}`);
  } else {
...
async function cmdBackup(): Promise<void> {
  ...
  const info = createBackup(ctx.paths, flags.has("--include-artifacts"));
...
async function cmdRestore(): Promise<void> {
  ...
  const freshCtx = { paths: new AppContext().paths }; // re-resolve paths only
  const result = restoreBackup(freshCtx.paths, name);
  console.log(`${pc.green("●")} Restored ${name}. A safety backup of the previous state is at ${result.safetyBackup.path}`);
}
...
To remove the code too, simply delete the repository folder afterwards.`);
  ...
  ctx.close();
  const paths = new AppContext().paths;
  ...
  console.log(`${pc.green("●")} Purged config/, logs/ and artifacts/. Skills, routines and connectors files were preserved.`);
}
...
async function cmdIndex(): Promise<void> {
  ...
  generateRouters(ctx.db, ctx.paths, ctx.settings());
  console.log(
    `${pc.green("●")} Indexed: ${stats.scanned} scanned, +${stats.added} added, ~${stats.updated} updated, -${stats.removed} removed, ${stats.skippedExcluded} excluded (${stats.durationMs} ms). Routers regenerated.`,
  );
  ctx.close();
}
...
async function cmdSync(): Promise<void> {
  const ctx = new AppContext();
  const target = positional[0] ?? ctx.paths.home;
  const plan = ctx.sync.plan(path.resolve(target));
  const approved = args.filter((a, i) => args[i - 1] === "--approve");
  console.log(pc.bold(`Sync plan for ${plan.targetDir}:`));
  for (const action of plan.actions) {
    const color =
      action.kind === "create" ? pc.green : action.kind === "update" ? pc.cyan : action.kind === "conflict" ? pc.red : pc.dim;
    console.log(` ${color(action.kind.padEnd(9))} ${action.filePath} ${pc.dim(action.reason)}`);
    if (action.kind === "conflict" && flags.has("--diff") && action.diff) {
      console.log(pc.dim(action.diff.split("\n").slice(0, 30).join("\n")));
    }
  }
  if (!flags.has("--apply")) {
    console.log(`\nDry run. Apply with: mordomo sync ${target} --apply` + (plan.conflicts ? "  (conflicts need --approve <file>)" : ""));
    ctx.close();
    return;
  }
  const result = ctx.sync.apply(plan, approved);
...
async function cmdRun(): Promise<void> {
  ...
  if (!slug) {
    console.error("Usage: mordomo run <skill-slug> [--provider claude|cursor|codex] [--model m] [--effort low|medium|high] [--input key=value]...");
    process.exit(1);
  }
  const ctx = new AppContext();
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === "--input" && args[i + 1]?.includes("=")) {
      const [k, ...v] = args[i + 1]!.split("=");
      inputs[k!] = v.join("=");
    }
  }
  const settings = ctx.settings();
  const provider = (getFlag("--provider") as ProviderId | undefined) ?? settings.defaultProvider;
  const run = ctx.runs.create({
    origin: "skill",
    provider,
    prompt: `(skill: ${slug})`,
    cwd: ctx.paths.home,
    model: getFlag("--model") ?? skill.recommendedModel ?? settings.providers[provider].defaultModel,
    effort: (getFlag("--effort") as Settings["providers"]["claude"]["defaultEffort"] | undefined) ?? skill.recommendedEffort,
  ...
  console.log(`${pc.cyan("▶")} Running ${skill.name} via ${provider} (run ${run.id})…`);
  const prompt = ctx.skills.buildRunPrompt(skill, inputs, path.join(ctx.paths.artifacts, run.id));
  const unsubscribe = ctx.runs.onEvent(run.id, (event) => {
...
// -------------------------------------------------------------- service ----
async function cmdService(): Promise<void> {
  const ctx = new AppContext();
  const sub = positional[0] ?? "plan";
  const plan = planStartupService(ctx.paths, process.execPath);
  if (sub === "plan") {
    ...
    await installStartupService(ctx, !flags.has("--yes"));
  } else if (sub === "remove") {
    for (const f of plan.files) {
      if (fs.existsSync(f.path)) fs.unlinkSync(f.path);
    }
    console.log("Service files removed. Also run:");
    for (const c of plan.uninstallCommands.filter((c) => !c.startsWith("rm "))) console.log(`  ${c}`);
    ctx.settingsStore.update({ autostart: false });
  }
  ctx.close();
}
