...
describe("approval payloads", () => {
  it("reads the launch input defensively", () => {
    expect(approvalTarget(approval({ payload: { kind: "prompt", input: { prompt: "hi", cwd: "/w", provider: "claude" } } }))).toEqual({
      kind: "prompt",
      skillSlug: null,
      prompt: "hi",
      cwd: "/w",
    ...
    expect(approvalTarget(approval({ payload: { kind: "skill", input: { slug: "tidy" } } })).skillSlug).toBe("tidy");
    expect(approvalTarget(approval({ payload: { kind: 7, input: 3 } }))).toEqual({ kind: "other", skillSlug: null, prompt: null, cwd: null, provider: null });
  });

  it("keeps only pending write-run approvals", () => {
    const list = [approval(), approval({ id: "a2", kind: "expose_port" }), approval({ id: "a3", status: "approved" })];
...
describe("approvalForRun", () => {
  ...
  const promptApproval = approval({ id: "ap", payload: { kind: "prompt", input: { prompt: "Rewrite the README please" } } });
  ...
  it("matches by skill slug and by prompt head", () => {
    ...
    expect(approvalForRun([promptApproval], run({ promptSummary: "Rewrite the README please" }))?.id).toBe("ap");
  ...
  it("labels rows", () => {
    ...
    expect(approvalLabel(approval({ payload: { kind: "prompt", input: { prompt: "line one\nline two" } } }), "?")).toBe("line one");
