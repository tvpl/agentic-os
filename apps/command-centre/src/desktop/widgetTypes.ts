...
export const cfgString = (c: WidgetConfig | undefined, key: string, fallback: string): string =>
  typeof c?.[key] === "string" ? (c[key] as string) : fallback;
export const cfgNumber = (c: WidgetConfig | undefined, key: string, fallback: number): number =>
  typeof c?.[key] === "number" && Number.isFinite(c[key]) ? (c[key] as number) : fallback;
export const cfgBool = (c: WidgetConfig | undefined, key: string, fallback: boolean): boolean =>
  typeof c?.[key] === "boolean" ? (c[key] as boolean) : fallback;
