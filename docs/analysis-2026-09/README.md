...
**Limite honesto.** O vídeo não pôde ser baixado em resolução plena (bloqueio do YouTube). A primeira versão desta análise usou a transcrição, a thumbnail e o storyboard. Depois, quatro gravações de tela fornecidas pelo usuário (celular filmando o vídeo em reprodução, 1206×2622, 36 a 60 fps, 11 a 22 s cada, cobrindo 1:50 a 2:45 do vídeo) permitiram extrair frames da interface a 1890×920 e sequências a 10 fps. A seção 1.5 e as revisões marcadas com "(gravações)" vêm dessa evidência. Durações de animação são estimadas contando frames a 10 fps; curvas de easing continuam inferidas.
...
3. **O Segundo Cérebro dele é uma ferramenta de navegação e de auditoria; o nosso ainda é sobretudo um mapa.** O painel de controles dele é idêntico ao nosso, item por item; a diferença está no que acontece ao clicar: preview em markdown com links navegáveis, relações do nó listadas por tipo (xlink, link, spoke), leque ordenado com subpastas como planetas. Ele substitui o Finder (busca instantânea, abrir no SO, preview), mostra arquivos como arcos concêntricos ordenados por departamento (legível com 35 mil arquivos), abre o conteúdo de uma skill e suas conexões ao clicar, e usa a densidade dos anéis como diagnóstico ("apps conectados demais", "rotinas que ninguém lembra"). O nosso descarta dois dos três tipos de aresta que a API já devolve (`world.ts:232-235`), não tem grafo local, backlinks, filtros por extensão/tag/data, nem estado na URL.
...
### 1.5 As gravações: animações e interatividade quadro a quadro

O que os frames a 1890×920 e as sequências a 10 fps mostram, e que o storyboard não mostrava.

**Dashboard**

- **Anel de artefatos.** 44 chips numerados em ordem de criação, cada um com ícone por tipo e uma contagem pequena no rodapé do chip (140, 149). Ao passar o mouse, uma etiqueta radial aparece com "nº · título" e, para saídas de skill, "nome · data, hora" (`/clean-up · 19 Aug, 07:0…`). O ícone de grade na barra de ferramentas do título **revela todas as etiquetas de uma vez** (frame 2:42:01). Dentro da esfera flutuam etiquetas dos runs mais recentes (`/games · 19 Aug, 07:02 pm`).
- **Núcleo.** Milhares de partículas coloridas por departamento em deriva lenta, dentro de um **poliedro em wireframe que gira em pseudo-3D**, sobre um padrão hexagonal estático. É o único movimento contínuo além do relógio; os widgets não pulsam.
- **Modo de busca** (ícone de lupa, sequência 2:42:09 a 2:42:15). Em cerca de 300 ms todos os widgets caem para ~30 % de opacidade, os chips perdem cor e um campo central aparece com o termo em caixa alta e a contagem "3 MATCHES". Só os chips que casam recebem anel laranja com brilho. Hover em um chip casado mostra um card com título e data ("OS RESTYLE - 10 BRAND CANDIDATES · Aug6 - 5:40am"). Clique abre um **modal central** com título, descrição, caminho e três botões (abrir arquivo, revelar na pasta, fechar). Enquanto o modal está aberto o anel encolhe e o núcleo desfoca: o fundo "recua" (depth push). Fechar reverte com a mesma transição.
- **Deck de skills.** Cada card mostra `/nome` e `MODELO · ESFORÇO` com o **modelo colorido** (SONNET azul, OPUS roxo, FABLE laranja); o cabeçalho diz "tap ▶ to run". A engrenagem abre um **popover ancorado ao card**, "MODEL + EFFORT": matriz HAIKU / SONNET / OPUS / FABLE × LOW / MEDIUM / HIGH / XHIGH / MAX em que uma barra colorida preenche de LOW até o esforço escolhido na linha do modelo escolhido. Ao clicar em outra linha a barra **desliza para a nova linha e troca de cor** em ~200 ms (sequência 2:36 do vídeo, frames 6 a 10 da gravação 1). É o mesmo mecanismo da nossa `ModelEffortMatrix`, mas inline, com cor por modelo e animado.
- **Rotinas.** Tabela com **ícone de runner por linha** (DESKTOP ou HERMES), linhas já disparadas esmaecidas, a próxima com o horário em badge laranja, "7/12 fired today" no cabeçalho e rodapé "CLAUDE DESKTOP 3 · HERMES 14 · DATA 20 AUG, 02:36 PM". Linhas reais: 11:00 deliverables status sweep (Hermes, fired), 13:00 community pulse digest (Hermes, fired), 15:00 invoice reconciliation (Desktop, next), 16:30 content pipeline check (Hermes, queued), 18:00 client report drafts (Hermes, queued), 20:00 evening ritual (Desktop, queued), 21:30 CRM enrich + next-day prep (Desktop, queued).
- **Calendário.** Relógio analógico com ponteiros e "Wk34 | Aug 20 2026 (Thu)", hora grande, "AEST · SYDNEY", **três fusos em colunas com dia da semana** (USA PT 09:41 pm WED · USA ET 12:41 am THU · LONDON 05:41 am THU), **grade de 4 × 13 quadrados (Q1 a Q4, uma célula por semana)** com a semana atual em laranja, e "WHAT'S NEXT" com três eventos e horários (Team standup 3:30pm, Partnership intro – Nordic SaaS 5:00pm, Team meet – sprint review 6:30pm).
- **YouTube Studio.** 170.000 inscritos, 8,5 M de views, "5 longforms this month", grade de pontos com publicações em laranja.
- **Email.** Além do já descrito, uma **barra segmentada "TODAY'S MIX"** com oito categorias (partners, leads, personal, other, LE, members, news, noise) e "SYNCED 02:40 PM · team@…".
- **Micro apps.** Linha com ícone, nome, descrição de uma frase e seta; é a lista de "apps" do OS.

**Segundo cérebro**

- **Painel de controles.** "Search 60,596 files… ( / )", LAYOUT Force / Circle / Hex / Rings, VIEW Departments / Folders, slider "RING SPIN (RINGS + HERMES)", checkbox "File names", sliders "Link springs 0.02" e "Circle / Hex size 0.59", Expand all / Collapse all, botão "Bake settings". Chip "◆ LEGEND" no canto inferior esquerdo e toast "Rings layout" no rodapé ao trocar layout. **Nosso painel em `views/SecondBrain.tsx:1040-1130` é o mesmo item por item, inclusive o "Bake".** A diferença está no que acontece ao interagir, não no painel.
- **Troca de layout.** Todos os nós fazem tween para a nova posição em cerca de 0,8 s com easing e os anéis surgem por fade (sequência 2:36:42 a 2:36:44). Nenhum salto.
- **Layout Rings.** Arcos concêntricos por departamento com pontos de tamanho variado, rótulo do hub com contagem ao lado (BUSINESS com 10999, 3574 e 2783 em subpastas; PRODUCT 3242), anel de rotinas com pontos âmbar e anel de aplicações com hexágonos com o logo do serviço (Telegram, Gmail, Notion, Drive, HubSpot, Slack, PayPal, Stripe, GitHub, Zapier, Cursor).
- **Clique em hub** (CONTENT). A câmera faz zoom no hub e os arquivos saem em **leque ordenado** com linhas radiais roxas e sobressalto de mola (~600 ms); subpastas aparecem como **"planetas" com anel e contagem** (99, 23). Clique em COMMUNITY faz o mesmo em azul e o leque anterior recolhe. É o nosso `toggleHub`, mas dirigido (leque ordenado, subpastas visíveis, zoom automático) em vez de aleatório (`Math.random()` em `SecondBrain.tsx:326-331`).
- **Clique em arquivo** (`CONTENT.md`). Painel lateral direito com o **markdown renderizado**: títulos de seção (CHOICE, PRODUCTION), passos numerados com bolinha de status verde, "Skills:" como chips de código, "Files:" e "Reference:" como links laranja. À esquerda, um card com "Open in editor", "Copy path" e a **lista de relações do nó por tipo: "xlink ×9", "link ×4", "spoke", "link"**. Ou seja, o RUBRIC tem arestas tipadas (xlink = ligação cruzada entre departamentos, link = link markdown, spoke = hub → arquivo) e as expõe no painel. **Nosso preview é um `<pre>` de texto puro** (`SecondBrain.tsx:1226`), e nossas arestas `same-dir` e `same-area` são descartadas antes de chegar ao canvas.
- **Layout Force.** Departamentos como blobs de partículas com o hub no centro e linhas finas entre hubs; a busca filtra em tempo real.
...
| Skills deck | 2×2, modelo colorido por família, popover ancorado com barra deslizante (gravações), run + config, "+ ADD SKILL" | Deck com matriz modelo × esforço em modal, favoritos, run | Paridade funcional; falta popover inline, cor por modelo e indicador "rodando agora" (`DeckWidget.tsx`) |
| Rotinas | Tabela hora · rotina · status com ícone de runner por linha, linhas disparadas esmaecidas, "7/12 fired today" (gravações) | Board Fired/Next/Queued/Paused, próximas 3 no "Hoje", validação de cron e preview | Paridade funcional; falta runner por linha, contagem do dia e agenda/timeline |
| Artefatos | Anel com 44 chips, etiquetas radiais reveláveis, **modo de busca** com dim global, contagem de matches, tooltip e modal abrir/revelar (gravações) | 3 últimos no painel "Agora" e chips orbitais sem busca; `/api/artifacts/recent` e `/file` | Falta busca, etiquetas, modal de detalhe e thumbnails |
| Segundo Cérebro | Setores por departamento com arcos; painel de controles idêntico ao nosso; **arestas tipadas (xlink, link, spoke)** listadas no painel; preview com **markdown renderizado e links**; leque ordenado com subpastas como planetas; abrir no SO; `brain.js` (gravações) | Mesmo painel; hubs com nebulosas aleatórias; preview em `<pre>`; só arestas `markdown-link`; Force/Círculo/Hex/Anéis; minimapa; cometas de runs ativas | Nosso tem cometas e minimapa; o dele é mais legível em escala e é navegação de verdade (links no preview, relações tipadas) |
...
1. **Arestas tipadas.** `/api/memory/graph` devolve `markdown-link`, `same-dir` e `same-area` com um `why` humano; `buildWorld` mantém só `markdown-link` (`world.ts:232-235`) e descarta tipo, direção e peso. Consequência: o grafo de arquivos sem links markdown (a maioria) fica sem aresta nenhuma. O RUBRIC mostra no painel do nó exatamente essa classificação ("xlink ×9, link ×4, spoke"), o que confirma que arestas tipadas são o que faz o grafo dele ler como sistema (gravações).
...
10. **Preview em texto puro.** `SecondBrain.tsx:1226` renderiza o arquivo em `<pre>`. No RUBRIC o preview de um router é markdown renderizado onde cada "Skills:" é um chip e cada "Files:" é um link; clicar navega no grafo. Para um router, que é uma lista de ponteiros, isso é a diferença entre ler e navegar (gravações).
11. **Legibilidade em escala**: nossas nebulosas por hub são bonitas até algumas centenas de arquivos; com 35 mil (o caso do vídeo) elas viram uma mancha. Os **arcos concêntricos por setor** do RUBRIC são uma função de layout a mais em `layouts.ts` (ordenar por `dir` e `mtime`, distribuir em arcos de raio crescente com passo fixo).
...
### 4.7 Acrescentado após as gravações

| # | Oportunidade | I | E | Onde |
|---|---|---|---|---|
| 55 | **Modo de busca no anel de artefatos**: dim global dos widgets, campo central com contagem de matches, brilho só nos chips casados, tooltip com título e data, modal com abrir / revelar na pasta / fechar, fundo recuando enquanto o modal está aberto | Alto | 1,5 | `desktop/Wallpaper.tsx:318-380`, novo `desktop/ArtifactSearch.tsx`, `apps/api/src/routes/system.ts` (`/api/artifacts/recent` com título e data) |
| 56 | **Etiquetas radiais nos chips** (nº · título, ou skill · data) e um toggle "revelar todas"; distinguir artefato de arquivo (a classe `.orbit-chip.artifact` já é usada e não existe) | Médio | 0,5 | `desktop/Wallpaper.tsx:355-380`, `theme.css` |
| 57 | **Popover modelo × esforço ancorado ao card**, com cor por família de modelo e barra que desliza entre linhas (200 ms) em vez do modal atual | Médio | 1 | `desktop/widgets/DeckWidget.tsx`, `desktop/SkillMatrixModal.tsx:45-90` |
| 58 | **Runner por rotina** (local / serviço / VPS) como ícone na linha, "n/m disparadas hoje" no cabeçalho, linhas já disparadas esmaecidas | Médio | 0,5 | `desktop/widgets/BoardWidget.tsx`, `views/Routines.tsx`, `core/src/routines/types.ts` |
| 59 | **Preview como markdown navegável**: renderizar `.md` com títulos, chips para `Skills:` e links para `Files:`/`Reference:` que selecionam o nó ou o orb correspondente no grafo; card lateral com relações por tipo (`markdown-link`, `same-dir`, `same-area`, e futuras) | Muito alto | 1,5 | `views/SecondBrain.tsx:1218-1240`, `core/src/memory/preview.ts` |
| 60 | **Explosão de hub dirigida**: leque ordenado por pasta e data, subpastas como planetas com contagem, zoom automático no hub, recolher o leque anterior; substituir `Math.random()` por posição determinística (testável no engine) | Alto | 1,5 | `views/SecondBrain.tsx:326-331`, `brain/engine/layouts.ts`, `brain/engine/world.ts` |
| 61 | **Depth push em modais e modos**: fundo em `scale(.97)` com leve blur e os widgets a 30 % de opacidade enquanto um modal, a busca ou o modo edição estão ativos; reverter com a mesma curva | Médio | 0,5 | `components/ui.tsx`, `theme.css`, `desktop/index.tsx` |
| 62 | **Três fusos em colunas com dia da semana** no widget "Hoje" (configuráveis, item 23) e grade Q1–Q4 por semana no lugar dos quarter dots | Baixo | 0,5 | `desktop/widgets/TodayWidget.tsx` |
| 63 | **Poliedro em wireframe pseudo-3D** girando devagar atrás do núcleo (projeção de um icosaedro, 12 vértices, sem WebGL), como textura de profundidade do desktop | Baixo | 0,5 | `desktop/Wallpaper.tsx:133-302` |
...
2. **Semanas 2 e 3**: 19, 20, 21, 22 (o centro do desktop passa a comandar), 44, 45, 47, 57 (custo, diff, indicador de execução, popover do deck), 61 (depth push).
3. **Semanas 4 e 5**: 31 a 37, 40, 59 e 60 (o cérebro vira navegação e diagnóstico), 25, 55 e 56 (artefatos com busca e etiquetas), 26 e 58 (agenda e runner).
